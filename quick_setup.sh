#!/bin/bash
#===============================================================================
# LigandMPNN MCP Quick Setup Script
#===============================================================================
# This script sets up the complete environment for LigandMPNN MCP server.
#
...
#   --skip-repo       Skip cloning LigandMPNN repository
...
echo -e "${BLUE}"
echo "=============================================="
echo "     LigandMPNN MCP Quick Setup Script       "
echo "=============================================="
echo -e "${NC}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --skip-repo       Skip cloning LigandMPNN repository"
...
echo ""
echo -e "${BLUE}Step 2: Cloning LigandMPNN repository${NC}"

if [ "$SKIP_REPO" = true ]; then
    info "Skipping repository clone (--skip-repo)"
elif [ -d "$REPO_DIR/LigandMPNN" ]; then
    info "LigandMPNN repository already exists"
else
    info "Cloning LigandMPNN repository..."
    mkdir -p "$REPO_DIR"
    git clone "$LIGANDMPNN_REPO" "$REPO_DIR/LigandMPNN"
...
if [ "$SKIP_ENV" = true ]; then
    ...
    info "Installing LigandMPNN requirements..."
    "${ENV_DIR}/bin/pip" install -r "$REPO_DIR/LigandMPNN/requirements.txt"
...
echo ""
echo -e "${BLUE}Step 4: Downloading model weights${NC}"

if [ "$SKIP_WEIGHTS" = true ]; then
    info "Skipping weights download (--skip-weights)"
elif [ -d "$REPO_DIR/LigandMPNN/model_params" ] && [ -f "$REPO_DIR/LigandMPNN/model_params/proteinmpnn_v_48_020.pt" ]; then
    info "Model weights already exist"
else
    info "Downloading model weights..."
    cd "$REPO_DIR/LigandMPNN"
...
"${ENV_DIR}/bin/python" -c "import torch; import numpy; print('Core packages OK')" && success "Core packages verified" || error "Package verification failed"
...
echo "Repository:  $REPO_DIR/LigandMPNN"
echo ""
...
echo "  claude mcp add ligandmpnn -- ${ENV_DIR}/bin/python ${SCRIPT_DIR}/src/server.py"
echo ""
