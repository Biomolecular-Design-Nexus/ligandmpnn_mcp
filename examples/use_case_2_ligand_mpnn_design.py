#!/usr/bin/env python3
"""
Use Case 2: Protein-Ligand Complex Design with LigandMPNN

This script demonstrates scaffold-based protein sequence generation in the context of
protein-ligand complexes. LigandMPNN considers ligand context when designing sequences,
generating sequences that are optimized for ligand binding.

Example usage:
python examples/use_case_2_ligand_mpnn_design.py --input examples/data/1BC8.pdb --output ./outputs/ligand_design
"""
...
def run_ligand_design(input_pdb, output_dir="./outputs/ligand_design", seed=111, temperature=0.1,
                     num_sequences=3, use_atom_context=True, use_side_chain_context=False):
    """
    Run protein sequence design using LigandMPNN for protein-ligand complexes

    Args:
        input_pdb: Path to input PDB file containing protein-ligand complex
        output_dir: Directory to save outputs
        seed: Random seed for reproducibility
        temperature: Sampling temperature (higher = more diversity)
        num_sequences: Number of sequences to generate
        use_atom_context: Whether to use ligand atoms as context
        use_side_chain_context: Whether to use fixed residue side chain atoms as context
    """
    ...
    class Args:
        def __init__(self):
            ...
            self.verbose = 1
            self.save_stats = 0
            self.ligand_mpnn_use_atom_context = 1 if use_atom_context else 0
            self.ligand_mpnn_use_side_chain_context = 1 if use_side_chain_context else 0
            # Model checkpoints
    ...
    print(f"Running LigandMPNN design on {input_pdb}")
    print(f"Using ligand atom context: {use_atom_context}")
    print(f"Using side chain context: {use_side_chain_context}")
    print(f"Output directory: {output_dir}")
    print(f"Generating {num_sequences} sequences with temperature {temperature}")

    try:
        run_main(args)
        print(f"✅ LigandMPNN design completed successfully! Check {output_dir} for results.")
        print(f"   - Generated sequences: {output_dir}/seqs/")
        print(f"   - Backbone structures: {output_dir}/backbones/")
        return True
    except Exception as e:
        print(f"❌ Error during LigandMPNN design: {e}")
        return False

def main():
    parser = argparse.ArgumentParser(description="Protein-ligand complex design using LigandMPNN")
    parser.add_argument("--input", "-i", required=True, help="Input PDB file with protein-ligand complex")
    parser.add_argument("--output", "-o", default="./outputs/ligand_design",
                       help="Output directory (default: ./outputs/ligand_design)")
    ...
    parser.add_argument("--no_ligand_context", action="store_true",
                       help="Disable ligand atom context (compare without ligands)")
    parser.add_argument("--use_side_chain_context", action="store_true",
                       help="Use fixed residue side chain atoms as extra ligand context")
    ...
    # Run the design
    success = run_ligand_design(
        input_pdb=args.input,
        output_dir=args.output,
        seed=args.seed,
        temperature=args.temperature,
        num_sequences=args.num_sequences,
        use_atom_context=not args.no_ligand_context,
