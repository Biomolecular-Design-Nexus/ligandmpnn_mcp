#!/usr/bin/env python3
"""
Script: ligand_design.py
Description: Protein-ligand complex design using LigandMPNN

Original Use Case: examples/use_case_2_ligand_mpnn_design.py
Dependencies Removed: Simplified Args class structure, externalized configuration

Usage:
    python scripts/ligand_design.py --input <input_file> --output <output_file>

Example:
    python scripts/ligand_design.py --input examples/data/1BC8.pdb --output results/ligand_design.csv
"""
...
def run_ligand_design(
    ...
    **kwargs
    ...
    """
    Run protein-ligand complex design using LigandMPNN.

    Args:
        input_file: Path to input PDB file containing protein-ligand complex
    ...
    Example:
        >>> result = run_ligand_design("examples/data/1BC8.pdb", num_sequences=5)
    ...
    # Override ligand context settings
    config["ligand_mpnn_use_atom_context"] = int(use_atom_context)
    config["ligand_mpnn_use_side_chain_context"] = int(use_side_chain_context)

    # Set output directory
    if output_file:
        output_dir = Path(output_file)
    else:
        output_dir = SCRIPT_DIR.parent / "results" / "ligand_design"
    ...
    try:
        # Run ligand design
        print(f"Running LigandMPNN design on {input_file}")
        print(f"Model type: {config['model_type']}")
        print(f"Output directory: {output_dir}")
        print(f"Generating {num_sequences} sequences with temperature {config['temperature']}")
        print(f"Ligand atom context: {'enabled' if use_atom_context else 'disabled'}")
        print(f"Side chain context: {'enabled' if use_side_chain_context else 'disabled'}")
...
def main():
    ...
    parser.add_argument('--no_atom_context', action='store_true',
                       help='Disable ligand atom context')
    parser.add_argument('--use_side_chain_context', action='store_true',
                       help='Enable side chain context')
    ...
    # Run
    result = run_ligand_design(
        input_file=args.input,
        output_file=args.output,
        num_sequences=args.num_sequences,
        use_atom_context=not args.no_atom_context,
