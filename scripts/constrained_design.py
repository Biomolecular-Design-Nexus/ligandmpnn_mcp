...
Description: Constrained protein sequence design with fixed/redesigned residues

Original Use Case: examples/use_case_5_constrained_design.py
Dependencies Removed: Simplified Args class structure, externalized configuration

Usage:
    python scripts/constrained_design.py --input <input_file> --output <output_file>

Example:
    python scripts/constrained_design.py --input examples/data/1BC8.pdb --output results/constrained_design.csv --fixed_residues "C1 C2 C3"
"""
...
def run_constrained_design(
    ...
    num_sequences: int = 3,
    fixed_residues: Optional[Union[str, List[str]]] = None,
    redesigned_residues: Optional[Union[str, List[str]]] = None,
    config: Optional[Dict[str, Any]] = None,
    **kwargs
) -> Dict[str, Any]:
    """
    Run constrained protein sequence design with fixed/redesigned residues.
    ...
    Args:
        ...
        redesigned_residues: Residues to redesign (e.g., "C4 C5" or ["C4", "C5"])
    ...
    Example:
        >>> result = run_constrained_design("examples/data/1BC8.pdb",
        ...                                fixed_residues="C1 C2 C3", num_sequences=5)
    ...
    if fixed_residues:
        if isinstance(fixed_residues, str):
            fixed_list = parse_residue_list(fixed_residues)
        elif isinstance(fixed_residues, list):
            fixed_list = fixed_residues

    redesigned_list = []
    if redesigned_residues:
        if isinstance(redesigned_residues, str):
            redesigned_list = parse_residue_list(redesigned_residues)
        elif isinstance(redesigned_residues, list):
            redesigned_list = redesigned_residues

    # Format for Args object
    fixed_str = format_residue_list(fixed_list)
    redesigned_str = format_residue_list(redesigned_list)

    # Set output directory
    if output_file:
        output_dir = Path(output_file)
    else:
        output_dir = SCRIPT_DIR.parent / "results" / "constrained_design"
    ...
    args = create_args_object(input_file, output_dir, num_sequences, fixed_str, redesigned_str, config)
    run_main = get_repo_runner()

    try:
        # Run constrained design
        print(f"Running constrained design on {input_file}")
        print(f"Model type: {config['model_type']}")
        print(f"Output directory: {output_dir}")
        print(f"Generating {num_sequences} sequences with temperature {config['temperature']}")

        if fixed_list:
            print(f"Fixed residues: {fixed_list}")
        if redesigned_list:
            print(f"Redesigned residues: {redesigned_list}")
        if not fixed_list and not redesigned_list:
            print("No constraints specified - will design all residues")
        ...
        return {
            ...
            "metadata": {
                ...
                "fixed_residues": fixed_list,
                "redesigned_residues": redesigned_list,
                **result["metadata"]
            }
        }
...
def main():
    ...
    parser.add_argument('--fixed_residues', help='Fixed residues (e.g., "C1 C2 C3")')
    parser.add_argument('--redesigned_residues', help='Redesigned residues (e.g., "C4 C5")')
